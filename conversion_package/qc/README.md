...
```powershell
# From the repo root (QT-arithmetic_study)
.\.venv_bids311\Scripts\python.exe -m pip install -r .\conversion_package\requirements-qc.txt
```

If you prefer PySide6 over PyQt5:

```powershell
.\.venv_bids311\Scripts\python.exe -m pip install matplotlib PySide6
```

## Run

```powershell
.\.venv_bids311\Scripts\python.exe .\conversion_package\qc\bids_signal_viewer.py
```

Optional arguments:

```powershell
.\.venv_bids311\Scripts\python.exe .\conversion_package\qc\bids_signal_viewer.py --bids-root bids_arithmetic
.\.venv_bids311\Scripts\python.exe .\conversion_package\qc\bids_signal_viewer.py --ecg-map conversion_package\config\ecg_channel_map.json
```
