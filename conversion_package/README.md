...
`bids_arithmetic` dataset from raw XDF files, plus a concise explanation of the
steps.
...
- Treat `bids_arithmetic/` as a build artifact; do not hand-edit files in this folder.
...
- Raw XDF files:
  - `Arithmetic_Data/sourcedata/*.xdf` (this repo includes the empty folder; place raw XDFs here)
...
`bids_arithmetic/sourcedata/`; use `--skip-copy-xlsx` to avoid that.

IDs in BIDS match the original XDF filenames (e.g., `001_arithmetic.xdf` -> `sub-001`).
...
- XDF filenames follow `NNN_arithmetic.xdf` (3-digit IDs).
- EEG stream name: `EEGStream`; pupil stream: `pupil_capture`.
- Marker stream: `arithmetic-Markers`; dropout stream: `UoHDataOffsetStream`.
- ECG channel is either D1 or D3 (map stored in `config/ecg_channel_map.json`).
- Trial duration: arithmetic = 6.0 s.
- Default line frequency is 50 Hz (override with `--line-freq` in Step 2 if needed).

## Quick start (venv)

From the repo root (`QT-arithmetic_study`), create a virtual environment and
...
```powershell
Remove-Item -Recurse -Force bids_arithmetic\sub-*
Remove-Item -Recurse -Force bids_arithmetic\sourcedata\xdf
```
...
python .\conversion_package\scripts\prepare_sourcedata_xdf.py `
  --raw-dir Arithmetic_Data\sourcedata `
  --bids-root bids_arithmetic `
  --identity `
  --report conversion_package\reports\participant_id_map_bids_arithmetic.tsv
```
...
python .\conversion_package\scripts\convert_xdf_to_bids_eeg.py `
  --xdf-dir Arithmetic_Data\sourcedata `
  --bids-root bids_arithmetic `
  --task arithmetic `
  --identity `
  --overwrite
...
```powershell
python .\conversion_package\scripts\extract_arithmetic_markers.py `
  --xdf-dir Arithmetic_Data\sourcedata `
  --bids-root bids_arithmetic
```
...
`bids_arithmetic/sourcedata/` unless `--skip-copy-xlsx` is set.
...
Ensure `bids_arithmetic/.bidsignore` includes:
...
npx bids-validator@1.15.0 bids_arithmetic

# Optional: capture validator outputs for release notes
npx bids-validator@1.15.0 bids_arithmetic > conversion_package\reports\bids_arithmetic_validator.txt
```
