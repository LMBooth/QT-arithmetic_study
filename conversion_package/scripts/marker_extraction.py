...
_ARITH_RE = re.compile(
    r"^(?P<range>\d+(?:\.\d+)?-\d+(?:\.\d+)?)\s+"
    r"(?P<outcome>Correct|Wrong)(?:\s+(?P<tutorial>tutorial))?$",
    re.IGNORECASE,
)
...
def _scrub_sys_path() -> None:
    ...
    sys.path = [p for p in sys.path if os.path.abspath(p) not in entries]
...
def build_arithmetic_events(
    ...
    *,
    ...
    for entry in entries:
        ...
        match = _ARITH_RE.match(marker)
        if match:
            range_label = match.group("range")
            outcome = match.group("outcome").capitalize()
            tutorial = match.group("tutorial")
            event["trial_type"] = range_label
            event["duration"] = ARITH_TRIAL_DURATION
            event["difficulty_range"] = range_label
            event["outcome"] = outcome
            event["response_accuracy"] = "1" if outcome == "Correct" else "0"
            event["istutorial"] = "true" if tutorial else "false"
        else:
...
def count_arithmetic_markers(
    entries: Iterable[MarkerEntry], *, file_label: str
) -> list[dict[str, Any]]:
    counts: dict[str, dict[str, int]] = {}
    for entry in entries:
        match = _ARITH_RE.match(entry.marker.strip())
        if not match:
            continue
        range_label = match.group("range")
        outcome = match.group("outcome").capitalize()
        if range_label not in counts:
            counts[range_label] = {"correct": 0, "wrong": 0}
        if outcome == "Correct":
            counts[range_label]["correct"] += 1
        else:
            counts[range_label]["wrong"] += 1

    rows: list[dict[str, Any]] = []
    for range_label in sorted(counts):
        correct = counts[range_label]["correct"]
        wrong = counts[range_label]["wrong"]
        total = correct + wrong
...
def write_arithmetic_summary_stats(path: Path, rows: list[dict[str, Any]]) -> None:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault(str(row["range"]), []).append(row)

    out_rows: list[dict[str, Any]] = []
    for range_label in sorted(grouped):
        group = grouped[range_label]
...
def extract_arithmetic(
    *,
    ...
    for xdf_path in xdf_paths:
        ...
        streams = load_xdf_streams(xdf_path, marker_name="arithmetic-Markers")
        ...
        counts_rows = count_arithmetic_markers(entries, file_label=file_label)
        counts_path = markers_dir / f"{participant}_counts_by_range.csv"
        if overwrite or not counts_path.exists():
            write_csv(
                counts_path,
                ["file", "range", "correct", "wrong", "total", "accuracy"],
                counts_rows,
            )
        ...
        if bids_stub_root is not None:
            if not entries:
                raise ValueError(f"No arithmetic markers in {xdf_path}")
            base_time = min(e.timestamp for e in entries)
            events = build_arithmetic_events(entries, base_time=base_time, marker_stream="arithmetic-Markers")
            events.sort(key=lambda e: float(e["onset"]))
            formatted = [format_event(e, missing_value="") for e in events]
            out_path = bids_stub_root / subject / "eeg" / f"{subject}_task-arithmetic_events.tsv"
            if overwrite or not out_path.exists():
                write_tsv(out_path, EVENT_FIELDS_BASE, formatted)

        if bids_root is not None:
            eeg_start = float(streams.eeg["time_stamps"][0])
            events = build_arithmetic_events(entries, base_time=eeg_start, marker_stream="arithmetic-Markers")
            ...
            out_path = bids_root / subject / "eeg" / f"{subject}_task-arithmetic_events.tsv"
            if overwrite or not out_path.exists():
                write_tsv(out_path, EVENT_FIELDS_ALL, formatted)

    counts_all_path = markers_dir / "counts_by_range_all_participants.csv"
    ...
    summary_path = markers_dir / "counts_by_range_summary_stats.csv"
    if overwrite or not summary_path.exists():
        write_arithmetic_summary_stats(summary_path, all_counts)
