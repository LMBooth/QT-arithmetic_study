...
def add_marker_stream_column(*, bids_root: Path, overwrite: bool) -> None:
    ...
    stream_for_trial = "arithmetic-Markers"
...
def update_events_json_definitions(*, bids_root: Path) -> None:
    ...
    for v in trial_types:
        ...
        elif range_re.match(v):
            trial_type_levels[v] = f"Arithmetic trial with difficulty_range={v}."
        else:
    ...
    for v in marker_streams:
        ...
        elif v == "arithmetic-Markers":
            marker_stream_levels[v] = "Task marker stream for the arithmetic paradigm."
        else:
    ...
    ensure_column("difficulty_range", "Arithmetic difficulty range (qmin-qmax) when applicable.")
